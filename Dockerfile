...
FROM public.ecr.aws/docker/library/ruby:3.3.1 as fetch-lib
WORKDIR /app
COPY --link Gemfile* ./
RUN apt-get update && apt-get install -y shared-mime-info libmariadb3
RUN bundle install

FROM public.ecr.aws/docker/library/ruby:3.3.1 as asset-compile
...
FROM public.ecr.aws/docker/library/ruby:3.3.1-slim
