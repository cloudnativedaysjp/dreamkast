...
FROM public.ecr.aws/docker/library/ruby:3.4.4 AS fetch-lib
WORKDIR /app
COPY --link Gemfile* ./
RUN apt-get update && apt-get install -y shared-mime-info libmariadb3
RUN bundle install

FROM public.ecr.aws/docker/library/ruby:3.4.4 AS asset-compile
...
FROM public.ecr.aws/docker/library/ruby:3.4.4-slim
