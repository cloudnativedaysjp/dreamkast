...
FROM node:24.12.0-slim AS node
WORKDIR /app
...
FROM public.ecr.aws/docker/library/ruby:4.0.0 AS fetch-lib
WORKDIR /app
COPY --link Gemfile* ./
RUN apt-get update && apt-get install -y shared-mime-info libmariadb3
RUN bundle install

FROM public.ecr.aws/docker/library/ruby:4.0.0 AS asset-compile
...
FROM public.ecr.aws/docker/library/ruby:4.0.0-slim
