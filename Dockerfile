...
FROM node:24.14.1-slim AS node
WORKDIR /app
...
FROM public.ecr.aws/docker/library/ruby:4.0.2 AS fetch-lib
WORKDIR /app
COPY --link Gemfile* ./
RUN apt-get update && apt-get install -y shared-mime-info libmariadb3
RUN bundle install

FROM public.ecr.aws/docker/library/ruby:4.0.2 AS asset-compile
...
FROM public.ecr.aws/docker/library/ruby:4.0.2-slim
