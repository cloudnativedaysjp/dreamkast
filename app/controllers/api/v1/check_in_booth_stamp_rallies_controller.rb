class Api::V1::CheckInBoothStampRalliesController < ApplicationController
  ...
  def create
    @params = check_in_booth_stamp_ralies_params(JSON.parse(request.body.read, { symbolize_names: true }))
    profile = Profile.find(@params[:profileId])
    check_in_timestamp = Time.zone.at(@params[:checkInTimestamp])
    @check_in = CheckInBoothStampRally.new(profile:, conference:, check_in_timestamp:)
  ...
  def check_in_booth_stamp_ralies_params(params)
