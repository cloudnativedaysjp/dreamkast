class SpeakerDashboard::SpeakersController < ApplicationController
  ...
  def pundit_user
    if current_user
      Speaker.find_by(conference_id: @conference.id, email: current_user[:info][:email])
    end
  end
  ...
  def talks_attributes
    attr = [:id, :title, :abstract, :document_url, :conference_id, :_destroy, :talk_category_id, :talk_difficulty_id, :talk_time_id, :sponsor_id, { talk_attributes: [] }]
    h = {}
