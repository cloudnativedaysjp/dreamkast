  ...
  def create
    ActiveRecord::Base.transaction do
      ...
      if @invite.save
        ...
        redirect_to(sponsor_dashboards_sponsor_contacts_path(id: @invite.sponsor_id))
      else
        flash[:alert] = "#{@invite.email} への招待メール送信に失敗しました: #{@invite.errors.full_messages.join(', ')}"
        redirect_to(new_sponsor_dashboards_sponsor_contact_invite_path(event: @conference.abbr, talk_id: @invite.talk_id))
      end
    end
  end
