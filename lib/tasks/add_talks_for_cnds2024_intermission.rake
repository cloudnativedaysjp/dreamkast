...
namespace :util do
  ...
  task add_talks_for_cnds2024_intermission: :environment do
    ...
    track_a_talks = [
      ...
      %w[18:00:00 18:10:00 クロージング intermission],
    ...
    other_track_talks = [
      ...
      %w[17:00:00 17:20:00 休憩 intermission],
      #  17:20    18:00    Session 5
      %w[18:00:00 18:10:00 トラックAでクロージング実施中！ intermission],
      %w[18:10:00 23:00:00 本日のイベントは終了しました intermission]
    ]
    ...
    track_a_talks.each do |arr|
      ...
      add_talk(param.merge(type: 'Intermission', conference_id: conference.id, conference_day_id: day.id, track_id: tracks['A'], show_on_timetable: true))
    end

    %w[B C].each do |track_name|
      other_track_talks.each do |arr|
        param = { start_time: arr[0], end_time: arr[1], title: arr[2], abstract: arr[3] }
        add_talk(param.merge(type: 'Intermission', conference_id: conference.id, conference_day_id: day.id, track_id: tracks[track_name], show_on_timetable: true))
      end
    end
  end
end
