namespace :util do
  desc 'generate_lead_cnds2025'
  task generate_lead_cnds2025: :environment do
    ActiveRecord::Base.logger = Logger.new($stdout)
    Rails.logger.level = Logger::DEBUG

    conference = Conference.find_by(abbr: 'cnds2025')
    track_viewer = TrackViewer.connection.execute("SELECT * FROM track_viewer WHERE created_at > '2025-05-22'")
      ...
      if target_sponsor_type.any?
        generated_csv = CSV.generate do |csv|
          ...
          Profile.where(conference_id: event_id).includes(:registered_talks).each do |profile|
            ...
            if s.abbr == 'ちゅらデータ' || s.abbr == 'Datadog Japan'
              unless profile.check_in_conferences[0].nil? || profile.check_in_conferences[0].created_at < '2025/05/23 03:30:00'
                checkin = 1
              end
            else
