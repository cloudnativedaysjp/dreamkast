...
RSpec.describe(SessionAttributeService, type: :service) do
  ...
  describe '.assign_attributes' do
    it 'is a convenience method that creates a new service instance' do
      expect_any_instance_of(SessionAttributeService).to(receive(:assign_attributes).with([keynote_attr.id]))
      SessionAttributeService.assign_attributes(talk, [keynote_attr.id])
    end
  end

  describe '#assign_attributes' do
    let(:service) { SessionAttributeService.new(talk) }
    ...
    context 'with valid attribute IDs' do
      ...
      it 'clears all attributes when empty array provided' do
        ...
        talk.reload
        expect(talk.keynote?).to(be(false))
        expect(talk.session_attributes).to(be_empty)
      end
    ...
    context 'with invalid attribute IDs' do
      it 'raises ValidationError for non-existent attribute IDs' do
        ...
        }.to(raise_error(SessionAttributeService::ValidationError, /Unknown attribute IDs: #{invalid_id}/))
      end

      it 'raises ValidationError for multiple exclusive attributes' do
        exclusive_attr_2 = create(:session_attribute, name: 'other_exclusive', is_exclusive: true)

        expect {
          service.assign_attributes([intermission_attr.id, exclusive_attr_2.id])
        }.to(raise_error(SessionAttributeService::ValidationError, /Multiple exclusive attributes/))
      end
      ...
      it 'raises ValidationError when exclusive attribute coexists with others' do
        ...
        }.to(raise_error(SessionAttributeService::ValidationError, /Exclusive attribute cannot coexist/))
      end

      it 'includes specific attribute names in error messages' do
        exclusive_attr_2 = create(:session_attribute, name: 'other_exclusive', display_name: 'Other Exclusive', is_exclusive: true)

        expect {
          service.assign_attributes([intermission_attr.id, exclusive_attr_2.id])
        }.to(raise_error(SessionAttributeService::ValidationError, /Other Exclusive/))
      end
    end
    ...
    context 'transaction behavior' do
      it 'rolls back all changes when validation fails' do
        ...
        talk.session_attributes << keynote_attr
        original_count = talk.talk_session_attributes.count

        # Try to add invalid combination (should fail)
        invalid_id = 99999
        expect {
          service.assign_attributes([sponsor_attr.id, invalid_id])
        }.to(raise_error(SessionAttributeService::ValidationError))

        # Verify no changes were made
        talk.reload
        expect(talk.talk_session_attributes.count).to(eq(original_count))
        expect(talk.keynote?).to(be(true))
    ...
    context 'edge cases' do
      ...
      it 'handles duplicate attribute IDs' do
        ...
        talk.reload
        expect(talk.keynote?).to(be(true))
        expect(talk.talk_session_attributes.count).to(eq(1)) # Should not create duplicates
      end
    end
  end
end
